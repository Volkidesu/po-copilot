...
# Custom Claude Code statusline - Created: 2026-06-20T17:53:21.539Z
...
burn_color() { if [ "$use_color" -eq 1 ]; then printf '\033[38;5;208m'; fi; }   # orange
usage_color() { if [ "$use_color" -eq 1 ]; then printf '\033[38;5;152m'; fi; }  # pale cyan
rst() { if [ "$use_color" -eq 1 ]; then printf '\033[0m'; fi; }
...
# ---- cost / token / burn-rate (native, from Claude Code's cost block) ----
cost_usd=""
cost_per_hour=""
tot_tokens=""
tpm=""
if [ "$HAS_JQ" -eq 1 ]; then
  ...
  tot_tokens=$(echo "$input" | jq -r '(((.context_window.total_input_tokens // 0) + (.context_window.total_output_tokens // 0))) | if . > 0 then . else empty end' 2>/dev/null)

  if [ -n "$cost_usd" ] && [ -n "$duration_ms" ] && [ "$duration_ms" != "0" ]; then
    cost_per_hour=$(echo "$input" | jq -r '(.cost.total_cost_usd // 0) / ((.cost.total_duration_ms // 1) / 3600000)' 2>/dev/null)
  fi
  if [ -n "$tot_tokens" ] && [ -n "$duration_ms" ] && [ "$duration_ms" != "0" ]; then
    tpm=$(echo "$input" | jq -r '(((.context_window.total_input_tokens // 0) + (.context_window.total_output_tokens // 0))) / ((.cost.total_duration_ms // 1) / 60000)' 2>/dev/null)
  fi
fi
